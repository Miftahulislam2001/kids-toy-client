...
const MyToys = () => {
    ...
    const url = `https://girls-toys.vercel.app/seller?email=${user?.email}`
    ...
    const deleteToy = (id) => {
        ...
        if (isDelete) {
            ...
            fetch(`https://girls-toys.vercel.app/delete/${id}`, {
                method: "DELETE"
            })
                .then(res => res.json())
                .then((data) => {
    ...
    useEffect(() => {
        fetch(`https://girls-toys.vercel.app/sort?sortby=${selectedOption.value}&email=${user.email}`)
            .then(res => res.json())
